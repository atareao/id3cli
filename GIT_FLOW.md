...
#    f) GitHub Actions compila binario Linux + SHA256
#    g) Crea GitHub Release con los artefactos
...
| `feat(scope): ...` | **minor** (0.Y.0) | `feat(cli): add --season flag` |
...
Gestiona el versionado en `Cargo.toml`.
...
### `ci.yml` (PR y push a development)

`cargo fmt --check` → `cargo clippy -D warnings` → `cargo build` → `cargo test`

### `release-prepare.yml` (push a main)

Detecta bump type, ejecuta vampus, genera changelog, crea tag, sincroniza `development`.

### `release.yml` (push tag v*)

Compila `x86_64-unknown-linux-gnu`, genera `.tar.gz` + SHA256, crea GitHub Release, publica en crates.io.
